...
const loadMembership=async(user:User|null)=>{if(!user||!supabase){setRole(null);setOrganizationId(null);return;}const{data}=await supabase.from("organization_members").select("organization_id,role").eq("user_id",user.id).eq("active",true).limit(1).maybeSingle();setRole((data?.role as Role)|null);setOrganizationId(data?.organization_id??null);};
