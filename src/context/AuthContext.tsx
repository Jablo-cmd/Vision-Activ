...
useEffect(()=>{if(!supabase){setLoading(false);return;}let mounted=true;supabase.auth.getSession().then(({data})=>{if(mounted)setSession(data.session);}).catch(()=>{if(mounted)setSession(null)});const{data}=supabase.auth.onAuthStateChange((_event,next)=>{if(mounted)setSession(next)});return()=>{mounted=false;data.subscription.unsubscribe()};},[]);
useEffect(()=>{let mounted=true;if(!session?.user){setRole(null);setOrganizationId(null);setLoading(false);return;}setLoading(true);loadMembership(session.user).catch(()=>{if(mounted){setRole(null);setOrganizationId(null)}}).finally(()=>{if(mounted)setLoading(false)});return()=>{mounted=false}},[session?.user?.id]);
