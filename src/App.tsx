  ...
  return <div className="flex min-h-screen bg-[#f6f8fb]">
    ...
    <div className="min-w-0 flex-1">
      <header className="sticky top-0 z-10 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur md:px-8">
        <div className="flex items-center gap-3"><Menu className="lg:hidden" size={20} /><div className="hidden items-center gap-2 rounded-xl bg-slate-50 px-3 py-2 md:flex"><Search size={16} className="text-slate-400" /><span className="text-sm text-slate-400">Search workspace</span></div></div>
        <div className="flex items-center gap-3"><Bell size={19} className="text-slate-500" /><button aria-label="Sign out" onClick={signOut} className="rounded-lg p-2 hover:bg-slate-100"><LogOut size={18} /></button><div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#071a35] text-sm font-bold text-white">{(user.email?.slice(0, 2) || "VA").toUpperCase()}</div></div>
      </header>
